...
-- Create a secure view that automatically applies the security filtering
...
WHERE
  -- Apply security filtering directly in the view definition
  cm.chat_id IN (
    SELECT id FROM chats 
    WHERE user_id = auth.uid() OR  -- User's own chat
        (
